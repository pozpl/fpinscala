...
object Option {
    ...
    def sequence[A](a: List[Option[A]]): Option[List[A]] = {
        def go(tail: List[Option[A]], acc: Option[List[A]]): Option[List[A]] = tail match {
//            case h :: t => h match {
//                case Some(a) => go(t, Some( a:: acc.getOrElse(Nil) ))
//                case _ => None
//            }
//            case Nil => acc.map( x => x.reverse )
            case h :: t => h.flatMap( hv => go(t, acc.map( accv => hv::accv )))
            case Nil => acc.map( x => x.reverse )
        }
