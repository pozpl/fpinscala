...
object Par {
    ...
    def sequence[A](ps: List[Par[A]]): Par[List[A]] = {
        ps.foldRight(unit(List[A]()))((p, acc: Par[List[A]]) => {
            map2(p, acc)(_ :: _)
        })
    }

    def parFilter[A](as: List[A])(p: A => Boolean): Par[List[A]] = {
        val predApl:List[Par[Option[A]]] = as.map(asyncF((a)=>if(p(a)) Some(a) else None))
        map(sequence(predApl))(_.filter(_.nonEmpty).map(_.get))
    }
